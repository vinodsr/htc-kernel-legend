			       ...
			       unsigned long flags)
{
	if ((flags & GPIOF_DRIVE_OUTPUT) && !vreg_h2w)
		vreg_h2w = vreg_get(0, LEGEND_H2W_POWER_NAME);
...
static struct gpio_chip legend_h2w_gpio_chip = {
	.start = LEGEND_GPIO_H2W_POWER,
	.end = LEGEND_GPIO_H2W_POWER,
...
void legend_init_h2w_power_gpio(void)
{
	register_gpio_chip(&legend_h2w_gpio_chip);
}
